...
export default async function handler(request, response) {
  ...
  // If no blob storage is configured, we cannot serve slug-based cards
  // (they would have been shared as embedded URLs instead)
  if (!hasBlobStorage()) {
    return response.status(404).json({ error: 'Card not found. Short-link storage is not configured on this deployment.' });
  }
